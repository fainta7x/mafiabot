    ...
    async with get_db() as conn:
        ...
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS users (
                ...
                nickname TEXT DEFAULT 'Не установлен', balance INTEGER DEFAULT 0,
                debt INTEGER DEFAULT 0, last_visit TEXT DEFAULT '-',
                total_paid INTEGER DEFAULT 0, has_unpaid_session INTEGER DEFAULT 0,
                games_played INTEGER DEFAULT 0, games_won INTEGER DEFAULT 0, points INTEGER DEFAULT 0
            )
        """)
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS evening_booking (
                user_id INTEGER PRIMARY KEY, status TEXT, date TEXT
            )
        """)
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS evening_history (
                id INTEGER PRIMARY KEY AUTOINCREMENT, date TEXT, user_id INTEGER,
                status TEXT, full_name TEXT, nickname TEXT, amount INTEGER DEFAULT 0
            )
        """)
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS evening_stats_messages (
                date TEXT PRIMARY KEY, chat_id INTEGER, message_id INTEGER
            )
        """)
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS evening_status (
                date TEXT PRIMARY KEY, bills_sent INTEGER DEFAULT 0
            )
        """)
        await conn.execute("""
        ...
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS game_slots_history (
                id INTEGER PRIMARY KEY AUTOINCREMENT, game_date TEXT, user_id INTEGER,
                slot_num INTEGER, role TEXT, team TEXT, base_points REAL DEFAULT 0,
                bonus_points REAL DEFAULT 0, lh_points REAL DEFAULT 0,
                will_protocol_points REAL DEFAULT 0, will_opinion_points REAL DEFAULT 0
            )
        """)
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS game_history (
                id INTEGER PRIMARY KEY AUTOINCREMENT, game_date TEXT,
                winner_label TEXT, protocol_text TEXT, game_number INTEGER,
...
get_current_game_date, set_current_game_date = lambda: get_setting("current_game_date"), lambda d: set_setting("current_game_date", d)
...
async def save_current_game_slots(slots: Dict[int, dict]):
    await set_setting("current_game_slots", json.dumps(slots, ensure_ascii=False))

async def load_current_game_slots() -> Optional[Dict[int, dict]]:
    payload = await get_setting("current_game_slots")
    return {int(k): v for k, v in json.loads(payload).items()} if payload else None

get_last_game_slots = load_current_game_slots
...
async def get_user_profile(user_id: int) -> Optional[Tuple]:
    async with get_db() as conn:
        async with conn.execute("SELECT full_name, nickname, debt, last_visit FROM users WHERE user_id = ?", (user_id,)) as cur:
...
async def add_booking(user_id: int, status: str, date: str):
    async with get_db() as conn:
        await conn.execute("INSERT OR REPLACE INTO evening_booking (user_id, status, date) VALUES (?, ?, ?)", (user_id, status, date))
        await conn.commit()
    ...
    async with get_db() as conn:
        async with conn.execute("""
            SELECT COALESCE(users.full_name, 'ID: ' || evening_booking.user_id) AS full_name,
                   users.username, users.nickname, evening_booking.status
            FROM evening_booking LEFT JOIN users ON evening_booking.user_id = users.user_id
        """) as cur:
            return await cur.fetchall()
...
async def get_booked_players_for_game() -> list:
    async with get_db() as conn:
        async with conn.execute("""
            SELECT e.user_id, IFNULL(u.full_name, 'Неизвестный') AS full_name,
                   u.username, u.nickname, e.status
            FROM evening_booking e LEFT JOIN users u ON e.user_id = u.user_id
...
async def count_ontime_players_for_date(date_str: str) -> int:
    async with get_db() as conn:
        async with conn.execute("SELECT COUNT(*) FROM evening_booking WHERE date = ? AND status = 'Вовремя'", (date_str,)) as cur:
            row = await cur.fetchone()
            return row[0] if row else 0

async def count_all_attending_for_date(date_str: str) -> int:
    async with get_db() as conn:
        async with conn.execute("SELECT COUNT(*) FROM evening_booking WHERE date = ? AND status IN ('Вовремя', 'Позже')", (date_str,)) as cur:
            row = await cur.fetchone()
            return row[0] if row else 0
...
async def get_players_by_status_for_date(date_str: str, status: str) -> list:
    async with get_db() as conn:
        async with conn.execute("""
            ...
            FROM evening_booking e LEFT JOIN users u ON e.user_id = u.user_id
            WHERE e.date = ? AND e.status = ? ORDER BY name
...
async def get_all_bookings_for_date_ordered(date_str: str) -> List[Tuple[str, str]]:
    async with get_db() as conn:
        async with conn.execute("""
            ...
            FROM evening_booking e LEFT JOIN users u ON e.user_id = u.user_id
            WHERE e.date = ?
            ORDER BY CASE e.status WHEN 'Вовремя' THEN 1 WHEN 'Позже' THEN 2 WHEN 'Не идёт' THEN 3 ELSE 4 END, name
        """, (date_str,)) as cur:
            return await cur.fetchall()

async def set_stats_message(date_str: str, chat_id: int, message_id: int):
    async with get_db() as conn:
        await conn.execute("INSERT OR REPLACE INTO evening_stats_messages (date, chat_id, message_id) VALUES (?, ?, ?)", (date_str, chat_id, message_id))
        await conn.commit()

async def get_stats_message(date_str: str) -> Optional[Tuple[int, int]]:
    async with get_db() as conn:
        async with conn.execute("SELECT chat_id, message_id FROM evening_stats_messages WHERE date = ?", (date_str,)) as cur:
            return await cur.fetchone()
...
async def add_user_payment(user_id: int, amount: int):
    async with get_db() as conn:
        await conn.execute("UPDATE users SET total_paid = COALESCE(total_paid, 0) + ? WHERE user_id = ?", (amount, user_id))
        await conn.commit()

async def get_debtors() -> list:
    async with get_db() as conn:
        async with conn.execute("SELECT full_name, nickname, username, debt, user_id FROM users WHERE has_unpaid_session = 1 ORDER BY full_name") as cur:
            return await cur.fetchall()
...
async def archive_current_evening():
    async with get_db() as conn:
        async with conn.execute("SELECT e.date, e.user_id, e.status, u.full_name, u.nickname FROM evening_booking e LEFT JOIN users u ON e.user_id = u.user_id") as cur:
            rows = await cur.fetchall()
        if rows:
            await conn.executemany("INSERT INTO evening_history (date, user_id, status, full_name, nickname, amount) VALUES (?, ?, ?, ?, ?, 0)", rows)
        await conn.execute("DELETE FROM evening_booking")
        await conn.commit()

async def get_evenings_list(limit: int = 10) -> list:
    async with get_db() as conn:
        async with conn.execute("SELECT date, COUNT(*) FROM evening_history GROUP BY date ORDER BY date DESC LIMIT ?", (limit,)) as cur:
            return await cur.fetchall()

async def get_evening_players(date_str: str) -> list:
    async with get_db() as conn:
        async with conn.execute("SELECT full_name, nickname, status, amount FROM evening_history WHERE date = ? ORDER BY full_name", (date_str,)) as cur:
            return await cur.fetchall()

async def get_top_players_by_visits(limit: int = 10) -> list:
    async with get_db() as conn:
        async with conn.execute("""
            SELECT u.full_name, u.nickname, COUNT(h.id) FROM evening_history h
            LEFT JOIN users u ON h.user_id = u.user_id GROUP BY h.user_id ORDER BY COUNT(h.id) DESC LIMIT ?
        """, (limit,)) as cur:
            return await cur.fetchall()
...
async def mark_evening_bills_sent(date_str: str):
    async with get_db() as conn:
        await conn.execute("INSERT INTO evening_status (date, bills_sent) VALUES (?, 1) ON CONFLICT(date) DO UPDATE SET bills_sent = 1", (date_str,))
        await conn.commit()
    ...
    async with get_db() as conn:
        for slot in slots.values():
            ...
            if team == winning_team:
                await conn.execute("UPDATE users SET games_won = COALESCE(games_won, 0) + 1, points = COALESCE(points, 0) + 1 WHERE user_id = ?", (uid,))
        await conn.commit()

async def get_user_game_counters(user_id: int) -> Optional[Dict[str, int]]:
    async with get_db() as conn:
        async with conn.execute("SELECT games_played, games_won, points FROM users WHERE user_id = ?", (user_id,)) as cur:
            row = await cur.fetchone()
            if not row:
                return None
    ...
    async with get_db() as conn:
        async with conn.execute("""
            SELECT role, COUNT(*) AS games,
                   ...
                   SUM(base_points + bonus_points + lh_points + will_protocol_points + will_opinion_points) AS total_points,
                   SUM(bonus_points) AS total_bonus, SUM(lh_points) AS total_lh,
                   SUM(CASE WHEN (bonus_points + lh_points) < 0 THEN bonus_points + lh_points ELSE 0 END) AS total_negative,
            ...
            FROM game_slots_history WHERE user_id = ? GROUP BY role
        """, (user_id,)) as cur:
            rows = await cur.fetchall()
...
async def save_game_history(game_date: str, winner_label: str, protocol_text: str,
    ...
    async with get_db() as conn:
        cur = await conn.execute("""
            INSERT INTO game_history (game_date, winner_label, protocol_text, game_number, global_game_number)
            VALUES (?, ?, ?, ?, ?)
...
async def get_user_games(user_id: int, limit: int = 10) -> List[Dict]:
    async with get_db() as conn:
        async with conn.execute("SELECT DISTINCT game_date FROM game_slots_history WHERE user_id = ? ORDER BY game_date DESC LIMIT ?", (user_id, limit)) as cur:
...
async def get_game_by_id(game_id: int) -> Optional[Dict]:
    games = await _fetch_games("SELECT id, game_date, winner_label, protocol_text, game_number, global_game_number FROM game_history WHERE id = ?", (game_id,))
